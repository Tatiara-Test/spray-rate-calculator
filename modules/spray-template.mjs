export const SPRAY_TEMPLATE = `
  ...
  <div class="spray-root">
    <main class="app-shell">
      ...
      <nav class="view-switch" aria-label="Spray Operations sections" role="tablist"><button id="spray-calculator-tab" class="selected" type="button" data-view-button="calculator" role="tab" aria-selected="true" aria-controls="calculator-view">Calculator</button><button id="spray-run-tab" type="button" data-view-button="run" role="tab" aria-selected="false" aria-controls="run-view">Run</button><button id="spray-paddocks-tab" type="button" data-view-button="paddocks" role="tab" aria-selected="false" aria-controls="paddocks-view">Paddocks</button></nav>
      ...
      <div id="calculator-view" data-view-panel="calculator" role="tabpanel" aria-labelledby="spray-calculator-tab">
        ...
        <button class="save-record-button" id="save-record-button" type="button">Save tank record</button><button class="secondary-record-button" id="start-run-from-calculator" type="button">Start paddock run</button><button class="clear-button" id="clear-button" type="button">Clear calculation</button>
      </div>
      <section id="run-view" data-view-panel="run" role="tabpanel" hidden aria-labelledby="spray-run-tab">
        <div class="run-intro"><div><p class="eyebrow">Controller-based allocation</p><h2>Multi-paddock run</h2><p>Record each controller boundary as you move between paddocks. The app allocates liquid and products without pretending Camera spray covered the whole paddock.</p></div></div>
        <section class="run-empty-card" id="run-empty-card">
          <strong>No run in progress</strong>
          <p id="run-calculation-status">Set up a tank mix in Calculator, then start a run.</p>
        ...
        <section class="active-run-card" id="active-run-card" hidden>
          <div class="active-run-heading"><div><p class="eyebrow">Active tank</p><h2 id="active-run-title">Run</h2></div><span id="active-run-method"></span></div>
          ...
          <form id="run-allocation-form" class="run-allocation-form">
            <div class="save-grid run-paddock-grid">
              <label class="dialog-field"><span>Paddock name</span><input id="run-paddock-name" list="paddock-suggestions" maxlength="60" autocomplete="off" required /></label>
              <label class="dialog-field"><span>Paddock size <small>optional</small></span><span class="input-with-unit compact-unit-input"><input id="run-paddock-size" type="number" inputmode="decimal" min="0" step="any" placeholder="e.g. 320" /><b>ha</b></span></label>
            </div>
          ...
          <div class="run-finish-actions"><button id="finish-run" type="button">Finish run</button><button id="cancel-empty-run" class="danger-button" type="button">Cancel empty run</button></div>
          <p class="run-helper">If the controller increases after a refill, finish this run and start a new one.</p>
        </section>
      </section>
      <section id="paddocks-view" data-view-panel="paddocks" role="tabpanel" hidden aria-labelledby="spray-paddocks-tab">
        ...
        <details class="archived-paddocks" id="archived-paddocks" hidden>
          ...
          <p>Archived controller-run records remain in this device's backups and can be restored.</p>
    ...
    <dialog class="save-dialog" id="save-dialog" aria-labelledby="save-dialog-title"><form method="dialog" class="save-panel" id="save-form">
      ...
      <div class="save-grid save-paddock-grid">
        <label class="dialog-field"><span>Paddock name</span><input id="save-paddock-name" list="paddock-suggestions" maxlength="60" autocomplete="off" required /></label>
        <label class="dialog-field"><span>Paddock size <small>optional</small></span><span class="input-with-unit compact-unit-input"><input id="save-paddock-size" type="number" inputmode="decimal" min="0" step="any" placeholder="e.g. 320" /><b>ha</b></span></label>
    ...
    <dialog class="save-dialog" id="run-start-dialog" aria-labelledby="run-start-dialog-title"><form method="dialog" class="save-panel" id="run-start-form">
      <div class="dialog-heading"><div><p class="eyebrow">New multi-paddock tank</p><h2 id="run-start-dialog-title">Start paddock run</h2></div><button class="close-dialog" type="button" id="close-run-start-dialog" aria-label="Close">&times;</button></div>
      ...
      <p class="dialog-error" id="run-start-error" hidden></p><button class="confirm-save-button" id="confirm-start-run" type="submit">Start run</button><button class="dialog-cancel-button" id="cancel-run-start" type="button">Cancel</button>
    </form></dialog>
